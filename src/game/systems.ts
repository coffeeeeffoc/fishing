...
export class Pool<T extends { alive: boolean }> {
  ...
  constructor(limit: number, factory: () => T) {
    this.limit = limit;
...
export class FishGrid {
  ...
  private eachCell(x1: number, y1: number, x2: number, y2: number, visit: (cell: Fish[]) => void) {
    ...
    for (
      let y = clamp(Math.floor(y1 / s) + 1, 0, this.rows - 1);
      y <= clamp(Math.floor(y2 / s) + 1, 0, this.rows - 1);
      y++
    )
      for (
        let x = clamp(Math.floor(x1 / s) + 1, 0, this.columns - 1);
        x <= clamp(Math.floor(x2 / s) + 1, 0, this.columns - 1);
        x++
      )
        visit(this.cells[y * this.columns + x]);
  }
  query(x1: number, y1: number, x2: number, y2: number) {
    ...
    this.eachCell(
      ...
      Math.max(y1, y2) + 7,
      (c) => {
        for (const f of c) this.found.add(f);
      },
    );
    return this.found;
  }
...
export function moveFish(f: Fish, dt: number, freeze: boolean) {
  ...
  f.invulnerable =
    f.kind === 'jelly' && (f.age + f.phase) % BEHAVIOR.jellyPeriod > BEHAVIOR.jellyInvincibleStart;
  let speed =
    spec.speed *
    (freeze ? GAME.freezeSlow : 1) *
    (f.slow > 0 ? (f.kind === 'shark' ? GAME.bossTailSlow : GAME.freezeSlow) : 1);
  if (f.inflated) speed *= BEHAVIOR.pufferSpeed;
  if (
    spec.behavior === 'pause' &&
    (f.age + f.phase) % BEHAVIOR.pufferPausePeriod < BEHAVIOR.pufferPauseDuration
  )
    speed *= BEHAVIOR.pufferPauseSpeed;
  if (
    spec.behavior === 'dash' &&
    (f.age + f.phase) % BEHAVIOR.swordPeriod > BEHAVIOR.swordDashStart
  )
    speed *= BEHAVIOR.swordDashSpeed;
  if (spec.behavior === 'accelerate')
    speed *= 1 + Math.min(f.age / BEHAVIOR.goldenAcceleration, BEHAVIOR.goldenMaxAcceleration);
  if (spec.behavior === 'turn') {
    if (f.age > BEHAVIOR.turtleTurnAfter && f.phase < 10) {
      f.dir *= -1;
      f.phase += 20;
    }
  }
  if (f.kind === 'shark') {
    ...
    if (f.x < 110) f.dir = 1;
    if (f.x > GAME.width - 110) f.dir = -1;
    f.y = 230 + Math.sin(f.age * 0.65) * 100;
  } else {
    const frequency = spec.behavior === 'wave' ? 2 : 0.95;
    const amplitude =
      spec.behavior === 'curve'
        ? 80
        : spec.behavior === 'wave'
          ? 42
          : spec.behavior === 'school'
            ? 16
            : 23;
  ...
  if (f.kind !== 'shark' && (f.x < -170 || f.x > GAME.width + 170 || f.age > BEHAVIOR.exitAge))
    f.alive = false;
}
...
export function fishSegmentHit(f: Fish, ax: number, ay: number, bx: number, by: number) {
  ...
  const rx = r * (f.kind === 'sword' ? 1.7 : 1.3) + 5,
    ry = r * (f.kind === 'shark' ? 0.42 : f.kind === 'sword' ? 0.53 : 1) + 5;
  return segmentHit(
    ((ax - f.x) * cos + (ay - f.y) * sin) / rx,
    (-(ax - f.x) * sin + (ay - f.y) * cos) / ry,
    ((bx - f.x) * cos + (by - f.y) * sin) / rx,
    (-(bx - f.x) * sin + (by - f.y) * cos) / ry,
    0,
    0,
    1,
  );
...
export function hitMultiplier(f: Fish, x: number, y: number) {
  ...
  const localY =
    (-dx * Math.sin(f.angle) + dy * Math.cos(f.angle)) * (Math.cos(f.angle) >= 0 ? 1 : -1);
